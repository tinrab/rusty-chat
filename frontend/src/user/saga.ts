...
import { ApiActionType, OutputError, OutputType, ReadApiAction } from '../api/types';
...
function* handleJoin(action: JoinUserAction): Generator<StrictEffect> {
    ...
    while (true) {
        ...
        if (read.payload.type === OutputType.Error) {
            yield put(userActions.joined({ error: true, code: read.payload.payload as OutputError }));
            break;
